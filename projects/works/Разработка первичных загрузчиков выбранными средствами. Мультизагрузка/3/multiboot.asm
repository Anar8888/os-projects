...
%include "fatTable.asm"	; Подключение таблицы с FAT-таблицей
...
FindNameFound:
  push si
  mov si, FoundMessage	;Записываем в стек указатель на строку
  call PrintString
  pop si
  push word [es:di+14h]
  push word [es:di+1Ah]
...
FileReadContinue:
  ...
  push si
  mov si, ReadClusterFinisedMessage	;Записываем в стек указатель на строку
  call PrintString
  pop si
  jc FileReadContinue ; Если не считали все, то продолжаем чтение
  
  push si
  mov si, RunMessage	;Записываем в стек указатель на строку
  call PrintString
  pop si
...
SelectMessage db "Select program:", 13, 10, "1.HELLO1.bin", 13,10, "2.HELLO2.bin", 13, 10, 0
ProgramName1	db	"HELLO1  BIN"   ; Имя исполняемой первой программы (11 символов)
ProgramName2	db	"HELLO2  BIN"   ; Имя исполняемой второй программы
ProgramName	db	"NNNNNN  BIN"   ; Имя исполняемой программы
ErrorMessage db "Error!", 13, 10, 0
SelectErrorMessage db "Bad select!", 13, 10, 0
FoundMessage db "File found!", 13, 10, 0
RunMessage db "Run of the program!", 13, 10, 0
ReadClusterFinisedMessage db "Read cluster finished!", 13, 10, 0
